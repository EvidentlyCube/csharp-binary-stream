...
describe("BinaryReader, number negative tests", () =>
{
	describe('OutOfBounds - numerics', () =>
	{
		it('readBoolean', () =>
		{
			...
			expect(() => reader.readBoolean()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(1, 0, 'readBoolean'));
		});

		it('readByte', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(0));
			expect(() => reader.readByte()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(1, 0, 'readByte'));
		});

		it('readSignedByte', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(0));
			expect(() => reader.readSignedByte()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(1, 0, 'readSignedByte'));
		});

		it('readShort (0-bytes remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(0));
			expect(() => reader.readShort()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(2, 0, 'readShort'));
		});
		it('readShort (1-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(1));
			expect(() => reader.readShort()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(2, 1, 'readShort'));
		});

		it('readUnsignedShort (0-bytes remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(0));
			expect(() => reader.readUnsignedShort()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(2, 0, 'readUnsignedShort'));
		});
		it('readUnsignedShort (1-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(1));
			expect(() => reader.readUnsignedShort()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(2, 1, 'readUnsignedShort'));
		});

		it('readInt (0-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(0));
			expect(() => reader.readInt()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(4, 0, 'readInt'));
		});
		it('readInt (1-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(1));
			expect(() => reader.readInt()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(4, 1, 'readInt'));
		});
		it('readInt (2-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(2));
			expect(() => reader.readInt()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(4, 2, 'readInt'));
		});
		it('readInt (3-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(3));
			expect(() => reader.readInt()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(4, 3, 'readInt'));
		});

		it('readUnsignedInt (0-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(0));
			expect(() => reader.readUnsignedInt()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(4, 0, 'readUnsignedInt'));
		});
		it('readUnsignedInt (1-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(1));
			expect(() => reader.readUnsignedInt()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(4, 1, 'readUnsignedInt'));
		});
		it('readUnsignedInt (2-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(2));
			expect(() => reader.readUnsignedInt()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(4, 2, 'readUnsignedInt'));
		});
		it('readUnsignedInt (3-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(3));
			expect(() => reader.readUnsignedInt()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(4, 3, 'readUnsignedInt'));
		});

		it('readLong (0-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(0));
			expect(() => reader.readLong()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(8, 0, 'readLong'));
		});
		it('readLong (1-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(1));
			expect(() => reader.readLong()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(8, 1, 'readLong'));
		});
		it('readLong (2-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(2));
			expect(() => reader.readLong()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(8, 2, 'readLong'));
		});
		it('readLong (3-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(3));
			expect(() => reader.readLong()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(8, 3, 'readLong'));
		});
		it('readLong (4-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(4));
			expect(() => reader.readLong()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(8, 4, 'readLong'));
		});
		it('readLong (5-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(5));
			expect(() => reader.readLong()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(8, 5, 'readLong'));
		});
		it('readLong (6-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(6));
			expect(() => reader.readLong()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(8, 6, 'readLong'));
		});
		it('readLong (7-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(7));
			expect(() => reader.readLong()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(8, 7, 'readLong'));
		});

		it('readLongString (0-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(0));
			expect(() => reader.readLongString()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(8, 0, 'readLongString'));
		});
		it('readLongString (1-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(1));
			expect(() => reader.readLongString()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(8, 1, 'readLongString'));
		});
		it('readLongString (2-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(2));
			expect(() => reader.readLongString()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(8, 2, 'readLongString'));
		});
		it('readLongString (3-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(3));
			expect(() => reader.readLongString()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(8, 3, 'readLongString'));
		});
		it('readLongString (4-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(4));
			expect(() => reader.readLongString()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(8, 4, 'readLongString'));
		});
		it('readLongString (5-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(5));
			expect(() => reader.readLongString()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(8, 5, 'readLongString'));
		});
		it('readLongString (6-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(6));
			expect(() => reader.readLongString()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(8, 6, 'readLongString'));
		});
		it('readLongString (7-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(7));
			expect(() => reader.readLongString()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(8, 7, 'readLongString'));
		});

		it('readUnsignedLong (0-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(0));
			expect(() => reader.readUnsignedLong()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(8, 0, 'readUnsignedLong'));
		});
		it('readUnsignedLong (1-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(1));
			expect(() => reader.readUnsignedLong()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(8, 1, 'readUnsignedLong'));
		});
		it('readUnsignedLong (2-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(2));
			expect(() => reader.readUnsignedLong()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(8, 2, 'readUnsignedLong'));
		});
		it('readUnsignedLong (3-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(3));
			expect(() => reader.readUnsignedLong()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(8, 3, 'readUnsignedLong'));
		});
		it('readUnsignedLong (4-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(4));
			expect(() => reader.readUnsignedLong()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(8, 4, 'readUnsignedLong'));
		});
		it('readUnsignedLong (5-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(5));
			expect(() => reader.readUnsignedLong()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(8, 5, 'readUnsignedLong'));
		});
		it('readUnsignedLong (6-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(6));
			expect(() => reader.readUnsignedLong()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(8, 6, 'readUnsignedLong'));
		});
		it('readUnsignedLong (7-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(7));
			expect(() => reader.readUnsignedLong()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(8, 7, 'readUnsignedLong'));
		});

		it('readUnsignedLongString (0-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(0));
			expect(() => reader.readUnsignedLongString()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(8, 0, 'readUnsignedLongString'));
		});
		it('readUnsignedLongString (1-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(1));
			expect(() => reader.readUnsignedLongString()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(8, 1, 'readUnsignedLongString'));
		});
		it('readUnsignedLongString (2-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(2));
			expect(() => reader.readUnsignedLongString()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(8, 2, 'readUnsignedLongString'));
		});
		it('readUnsignedLongString (3-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(3));
			expect(() => reader.readUnsignedLongString()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(8, 3, 'readUnsignedLongString'));
		});
		it('readUnsignedLongString (4-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(4));
			expect(() => reader.readUnsignedLongString()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(8, 4, 'readUnsignedLongString'));
		});
		it('readUnsignedLongString (5-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(5));
			expect(() => reader.readUnsignedLongString()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(8, 5, 'readUnsignedLongString'));
		});
		it('readUnsignedLongString (6-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(6));
			expect(() => reader.readUnsignedLongString()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(8, 6, 'readUnsignedLongString'));
		});
		it('readUnsignedLongString (7-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(7));
			expect(() => reader.readUnsignedLongString()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(8, 7, 'readUnsignedLongString'));
		});

		it('readFloat (0-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(0));
			expect(() => reader.readFloat()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(4, 0, 'readFloat'));
		});
		it('readFloat (1-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(1));
			expect(() => reader.readFloat()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(4, 1, 'readFloat'));
		});
		it('readFloat (2-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(2));
			expect(() => reader.readFloat()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(4, 2, 'readFloat'));
		});
		it('readFloat (3-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(3));
			expect(() => reader.readFloat()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(4, 3, 'readFloat'));
		});

		it('readDouble (0-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(0));
			expect(() => reader.readDouble()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(8, 0, 'readDouble'));
		});
		it('readDouble (1-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(1));
			expect(() => reader.readDouble()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(8, 1, 'readDouble'));
		});
		it('readDouble (2-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(2));
			expect(() => reader.readDouble()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(8, 2, 'readDouble'));
		});
		it('readDouble (3-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(3));
			expect(() => reader.readDouble()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(8, 3, 'readDouble'));
		});
		it('readDouble (4-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(4));
			expect(() => reader.readDouble()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(8, 4, 'readDouble'));
		});
		it('readDouble (5-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(5));
			expect(() => reader.readDouble()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(8, 5, 'readDouble'));
		});
		it('readDouble (6-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(6));
			expect(() => reader.readDouble()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(8, 6, 'readDouble'));
		});
		it('readDouble (7-byte remaining)', () =>
		{
			const reader = new BinaryReader(getBufferOfLength(7));
			expect(() => reader.readDouble()).to.throw(OutOfBoundsError, OutOfBoundsMessageFactory.notEnoughBytesInBuffer(8, 7, 'readDouble'));
		});
	});

	describe('OutOfBounds - readChar(utf8)', () =>
	{
	...
	describe('OutOfBounds - readString(utf8)', () =>
	{
